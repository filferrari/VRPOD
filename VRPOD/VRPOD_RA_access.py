...
def solve_VRPOD_access(instance: Instance):
    ...
    for i in range(1, instance.C +1):
        for k in range(1, instance.K +1):
            OD = instance.C + k
            if (instance.v[k - 1] == 0):
                od_original_route = instance.d_sd[0, OD]
                od_delivery_route = instance.d_sd[0, i] + instance.d_sd[i, OD]
                chargeable_distance = od_delivery_route - od_original_route

                p[i, OD] = instance.rho * (od_delivery_route - od_original_route)

            else:
                od_original_route = instance.d[0, OD]
                od_delivery_route = instance.d[0, i] + instance.d[i, OD]
                chargeable_distance = od_delivery_route - od_original_route
                p[i, OD] = instance.rho * (od_delivery_route - od_original_route) + instance.T * (
                        instance.t[0, i] + instance.t[i, OD] -
                        instance.t[0, OD]) * instance.v[k - 1]

            if (od_delivery_route <= instance.max_dev * od_original_route) and ((instance.q[i]+instance.m[k-1]) <= instance.l[k-1]):
                beta[i, OD] = 1
            else:
                beta[i, OD] = 0
    m.update()
    ...
    RD_costs = LinExpr()
    #for node_i, node_j in b.keys():
    for node_i, node_j in instance.dRD.keys():
        RD_costs += x[node_i, node_j] * (instance.d[node_i, node_j] * instance.CF + instance.t[node_i, node_j] * instance.T)
    ...
    if m.Status == gp.GRB.OPTIMAL:
        ...
        for node_i, node_j in x:
            ...
            rd_distance_RA += instance.t_dist[node_i, node_j]*(x[node_i, node_j].X >= m.params.IntFeasTol) * instance.d[node_i, node_j]

            rd_timestoll += instance.t[node_i, node_j] * (x[node_i, node_j].X >= m.params.IntFeasTol)
        ...
        for i in range(1, instance.C +1):
            ...
            for k in range(1, instance.K+1):
                ...
                if w[i, OD].X >= m.params.IntFeasTol:
                    ...
                    distance_od += instance.d[0, i] + instance.d[i, OD] - instance.d[0, OD]

                    od_distance_RA += instance.t_dist[0, i] * instance.d[0, i] + instance.t_dist[i, OD] * instance.d[i, OD] - instance.t_dist[0, OD] * instance.d[0, OD]

        return solution_rd, m.ObjVal, RD_costs.getValue(), OD_compensation.getValue(), customers_rd, customers_od, distance_rd, distance_od, rd_timestoll, od_timestoll, rd_distance_RA, od_distance_RA
